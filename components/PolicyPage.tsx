    ...
    <main role="main" className="min-h-screen bg-[var(--site-bg)] text-[#1f1a14]">
      ...
      <section role="region" aria-label="Policy header" className="relative overflow-hidden bg-[var(--site-bg)] py-10 sm:py-14">
        ...
        <div className="relative mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
          <Link
            href="/contact"
            className="inline-flex items-center gap-2 rounded-full border border-[#d4af37]/25 bg-white px-4 py-2 text-sm font-bold text-[#1f1a14] shadow-sm transition hover:border-[#d4af37] hover:bg-[#fff7e6]"
          >
          ...
          <div className="mt-8 flex flex-col gap-5 sm:flex-row sm:items-start">
            <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl border border-[#d4af37]/30 bg-[#fff7e6] text-[#d4af37]">
              <FileText className="h-6 w-6" />
            </div>
            <div>
              <p className="text-xs font-black uppercase tracking-[0.28em] text-[#6f1d1b]">
                {policy.label}
              </p>
              <h1 className="mt-3 text-4xl font-black text-[#1f1a14] sm:text-5xl">
                {policy.title}
              </h1>
              <p className="mt-4 max-w-3xl text-base leading-7 text-[#7a6a55] sm:text-lg">
                {policy.summary}
              </p>
              <p className="mt-4 text-sm font-bold text-[#7a6a55]">
      ...
      <section role="region" aria-label="Policy content" className="bg-[var(--site-bg)] py-10 sm:py-14">
        <div className="mx-auto grid max-w-5xl gap-6 px-4 sm:px-6 lg:px-8">
          ...
          {policy.sections.map((section) => (
            <article
              key={section.title}
              className="rounded-[28px] border border-[#d4af37]/20 bg-white p-6 shadow-[0_18px_60px_rgba(31,26,20,0.06)] sm:p-8"
            >
              <h2 className="text-2xl font-black text-[#1f1a14]">{section.title}</h2>
              <div className="mt-5 space-y-4">
                {section.body.map((item) => (
                  <div key={item} className="flex gap-3 text-sm leading-7 text-[#7a6a55] sm:text-base">
